...
description: 用于 OpenClaw 或其他智能体调用 OpenClaw-MiroSearch。覆盖安装部署（Docker Compose 或 uv）、API 调用（优先 run_research_once_v2，兼容 v1）、模式与检索路由选择（mode + search_profile + 检索深度参数）、以及卡住/超时/限流的降级重试排障。
---
