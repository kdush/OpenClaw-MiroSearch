...
description: 用于 OpenClaw 或其他智能体调用 OpenClaw-MiroSearch。覆盖安装部署（Docker Compose 或 uv）、API 调用（run_research_once/run_research_once_v2/stop_current/info）、模式与检索路由选择（mode + search_profile + 检索深度参数）、常见卡住与超时排障。
---
...
1. 优先使用 `run_research_once_v2` 发起研究并轮询结果：`references/api.md`
...
- 对核查类问题给出 `search_result_num` 与 `verification_min_search_rounds`
- 若失败，返回“失败点 + 下一步排查命令”
