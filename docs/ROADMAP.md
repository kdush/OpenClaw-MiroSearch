...
- **研究报告引用可点击**：Gradio Web UI 中研究总结正文的 `[N]` 引用自动转换为指向报告末尾"References / 参考文献"真实 URL 的 HTML 锚点（新标签页打开），支持多种参考文献标题样式，跳过代码块，不改动参考文献章节本身
- **Compose Worker 启动修复**：`api-worker` 服务启动命令改为 `.venv/bin/python worker.py`，避免容器内 `PATH` 误解析导致 arq Worker 启动异常

版本定位：

- 面向体验与部署稳定性的 patch 版本，不改变 API 契约与数据结构

### `v0.2.2`（API 模式 + 断电重连 + 抓取雏形）
...
### `v0.2.11`（全局契约与安全收敛 · 当前版本）

已纳入能力：

- **最终输出契约收敛**：统一总结证据保留、格式质量与 Pipeline 映射结果协议，失败任务不再伪装成功
- **API 参数全链路一致**：请求、缓存、任务元数据、队列与 Worker 使用同一组有效参数，模式硬预算覆盖篇幅策略
- **Gradio 调用隔离**：API 后端分流、缓存深度参数与 `caller_id` 定向取消保持一致，每个任务独立持有 ToolManager
- **分阶段模型路由**：OpenAI 与 Anthropic 共用阶段模型和 token 上限语义
- **检索配额韧性**：SerpAPI、Serper 与 Tavily 支持多 Key 轮转、429 冷却和有界重试
- **安全部署契约**：鉴权保持 fail-closed，Compose 与文档明确区分开发绕过和生产 Token 模式

版本定位：

- 面向运行时正确性、调用隔离和安全默认值的 patch 版本；完整变更见 [`docs/CHANGELOG.md`](./CHANGELOG.md)

## 后续规划

> `v0.2.3` 至 `v0.2.10` 的实际交付记录以 [`docs/CHANGELOG.md`](./CHANGELOG.md) 为准；以下保留尚未完成的长期规划背景。
...
1. ~~**异步任务队列**（v0.2.0）~~ — ✅ 已完成
1. **Eval Pipeline CI 化**（v0.2.5）— 质量提升的度量基础，没有评测就没有可量化的改进
1. **Prometheus 可观测性**（v0.2.5）— 生产运行必备，发现瓶颈和异常的基础设施
