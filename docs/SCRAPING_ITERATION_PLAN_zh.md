...
更新时间：2026-08-13
仓库当前版本：`v0.2.11`
抓取专项交付版本：`v0.2.5`（T1 / T2 / T3 / T4 / T5 / T6 / T7 / T8 工具改造已发布；E 类 prompt 协同尚未完成；T9 待实现）
...
| A2 | PDF 直接被拒 | 使用 `pdfminer.six` 解析 PDF 文本，保留段落边界 |
...
| ID | 现状 | 期望 |
|----|------|------|
| C1 | 每次调用都新建 `httpx.AsyncClient` | 共享单例 client，复用连接池；记录请求 / 解析 / 抽取阶段 metrics |
...
> 注：E 类改造与 prompt 工程绑定，不作为 T1-T9 的独立任务跟踪，目前尚未完成，仍属于后续工作；T1-T8 的已发布状态仅指下文列出的抓取工具改造。
...
- 测试：
  - 响应 header 指定 GBK 时，正文中文可正确解码
  - 响应 header 未指定 charset 时，使用 `<meta charset>` 兜底
...
- 设计：
  ...
  - RSS / Atom：使用 `xml.etree.ElementTree` 解析，返回 `feed_title`、`entries: [{title,link,published,summary}]`，最多前 50 条
...
- 回滚：去掉新加 mime 走老路径
- 风险：格式不规范的 XML 可能无法通过 `ElementTree` 解析；解析前需先走 T4 解码路径
...
1. **集成验证**：在 demo / api-server 重跑下列 query，要求最终 Markdown 必须包含从 `scrape_url` 抓回的 **原文片段** 而不是仅 `google_search` snippet：
   - 监管类："深圳公交吸烟事件中应用了哪些条例？请给出条例原文"
   - 统计类："2024 年深圳市常住人口数据出处与具体数字"
   - 公告类："国家市场监管总局 2026 年 1 月发布的合规处罚案例"
1. **回归门禁**：在 CI `run-tests.yml` 中追加 `scrape_url` mock 测试用例，禁止重定向 SSRF 与超大 body 通过。
1. **观测性**：每个版本灰度上线后查看 `stage_heartbeat` 中是否出现 `scrape_url`，命中率 / 平均耗时通过 worker 日志聚合。
...
| "OpenAI 2026 Q1 财报"（T9 规划 Demo） | `google_search` → `scrape_urls([官网,SEC,新闻])` | T9 发布后，报告交叉引用 ≥3 源，用 `[N]` 引用真实 URL |
...
- E 类待办：更新 `apps/miroflow-agent/src/prompts/`，使 `google_search` 命中长正文需求时至少调用一次 `scrape_url`。该 prompt 行为不属于已发布的 T1-T8 工具改造。
