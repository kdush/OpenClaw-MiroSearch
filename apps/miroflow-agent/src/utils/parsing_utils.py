...
def extract_failure_experience_summary(text: str) -> str:
    ...
    cutoff_match = re.search(r"<use_mcp_tool>|<longcat_tool_call>", after_think)
    if cutoff_match:
        content = after_think[: cutoff_match.start()].strip()
    else:
...
def extract_llm_response_text(llm_response: Union[str, Dict]) -> str:
    ...
    match = re.search(r"<use_mcp_tool>|<longcat_tool_call>", content)
...
def parse_llm_response_for_tool_calls(
    ...
    tool_calls = []
    longcat_tool_call_patterns = re.findall(
        r"<longcat_tool_call>\s*([\s\S]*?)\s*</longcat_tool_call>",
        llm_response_content_text,
        re.DOTALL,
    )
    for call_json in longcat_tool_call_patterns:
        parsed_call = safe_json_loads(call_json.strip())
        if not isinstance(parsed_call, dict):
            continue
        full_name = (parsed_call.get("name") or "").strip()
        arguments = filter_none_values(parsed_call.get("arguments") or {})
        if "-" in full_name:
